// ============================================================
// 系统10 结局：曝光阶梯 tier × 事业等级 × 恋爱深度 → 细分结局（10+）
// 含 情敌隐藏结局（女主对男主好感<40 + 情敌倾向>=50 + 接受情敌告白）
// ============================================================
import { GS } from '../state.js';
import { getExposureTier } from './public-opinion.js';
import { popularity, careerLevel, maleLead } from './state.js';
import { affectionStageIndex, affectionDepth } from './romance.js';

// 结局矩阵：舆论 tier + 事业 + 恋爱深度
function pickEnding(tier, careerLv, depth, rivalQuit) {
  // 情敌隐藏结局优先判定
  var rivalNode = GS.entSim.npcNetwork.nodes['npc_suitor'];
  var rivalLean = rivalNode ? (rivalNode.intimacy || 0) : 0;
  // 团内竞争者隐藏结局：女主对男主好感 < 40 + 竞争者亲密度 >= 50
  if (rivalLean >= 50 && GS.entSim.affection < 40) {
    return { type: 'hidden_rival', label: '藏在掌声里的他', text: '你最终走向了那个一直站在你身边的人，男主成了回忆里最亮的光。', icon: '💞' };
  }
  if (depth >= 5) {
    return tier === '引爆' || tier === '哗然'
  ...
  if (depth >= 3) {
    if (tier === '引爆' || tier === '哗然') return { type: 'be_exposed', label: '曝光塌房', text: '越藏越险，最终一拍即塌。', icon: '⛈️' };
    if (tier === '安全') return { type: 'he_underground', label: '地下永久', text: '事业巅峰，爱情藏得滴水不漏，你们在无人处相拥。', icon: '🌙' };
  ...
  // 恋爱深度 0：看事业
  if (careerLv === '传奇' || careerLv === '顶流') return { type: 'he_solo', label: '独美封神', text: '你把所有热爱给了舞台，单身亦封神。', icon: '👑' };
  if (tier === '引爆' || tier === '哗然') return { type: 'be_fall', label: '黯然退场', text: '舆论反噬，你悄悄退出了聚光灯。', icon: '🌑' };
